...
class ServerRepository implements ServerRepositoryInterface
{
    ...
    {
        if($useCache){
            $cachedData = $this->cache->get(CacheManager::SERVER_LIST_KEY, CacheManager::SERVER_LIST_KEY);
            if (! empty($cachedData)) {
                $this->data = $cachedData;
            }else{
                $this->loadData();
                $this->cache->set(CacheManager::SERVER_LIST_KEY, CacheManager::SERVER_LIST_KEY, $this->data);
            }
        }else{
            $this->loadData();
        }
    ...
    {
        if($useCache){
            $cachedData = $this->cache->get(CacheManager::LOCATION_LIST_KEY, CacheManager::LOCATION_LIST_KEY);
            if (! empty($cachedData)) {
                return $cachedData;
            }else{
                $this->loadData();
                $this->cache->set(CacheManager::LOCATION_LIST_KEY, CacheManager::LOCATION_LIST_KEY, $this->locationData);
            }
        }else{
            $this->loadData();
        }
